...
import { Shield, ShieldAlert, CheckCircle, XCircle, Trash2, ArrowLeft, Download, Clock, RefreshCw, History, Eye, X, Activity, Database, Fingerprint, Zap, Lock as LockIcon } from 'lucide-react';
...
export default function RepositoryDetails() {
    ...
    const handleDownloadPDF = async () => {
        ...
        try {
            ...
            // Deduplicate
            const uniqueVulns = new Map<string, TrivyVulnerability>();
            report.Results?.forEach((res: TrivyResult) => {
            ...
            let vulnerabilities = Array.from(uniqueVulns.values());

            // Sort by Severity (Critical -> High -> Medium -> Low)
            const severityWeight: Record<string, number> = { 'CRITICAL': 4, 'HIGH': 3, 'MEDIUM': 2, 'LOW': 1, 'UNKNOWN': 0 };
            vulnerabilities.sort((a, b) => (severityWeight[b.Severity] || 0) - (severityWeight[a.Severity] || 0));

            // Calculate Summary
            const summary = { CRITICAL: 0, HIGH: 0, MEDIUM: 0, LOW: 0, TOTAL: vulnerabilities.length };
            vulnerabilities.forEach(v => {
                if (summary[v.Severity as keyof typeof summary] !== undefined) {
            ...
            doc.text("Security Vulnerability Audit", 14, 20);

            doc.setFontSize(10);
            doc.setTextColor(100);
            doc.text(`Generated by RegistryX Intelligence`, 14, 26);
            ...
            autoTable(doc, {
                startY: 45,
                head: [['Context', 'Artifact', 'Tag', 'Scan Date', 'Total Risks']],
                body: [['RegistryX', name, selectedTag, new Date().toLocaleString(), summary.TOTAL.toString()]],
                theme: 'plain',
            ...
            autoTable(doc, {
                ...
                head: [['Severity Distribution', 'CRITICAL', 'HIGH', 'MEDIUM', 'LOW']],
                body: [['Count', summary.CRITICAL, summary.HIGH, summary.MEDIUM, summary.LOW]],
                headStyles: { fillColor: [50, 50, 50], textColor: 255, fontStyle: 'bold' },
                columnStyles: {
                    1: { textColor: [220, 38, 38], fontStyle: 'bold' },
                    2: { textColor: [234, 88, 12], fontStyle: 'bold' },
                    3: { textColor: [202, 138, 4], fontStyle: 'bold' },
            ...
            doc.setFontSize(14);
            doc.setTextColor(0);
            doc.text("Detailed Findings", 14, (doc as any).lastAutoTable.finalY + 15);

            const tableData = vulnerabilities.map((v: TrivyVulnerability) => [
                v.Severity,
                v.PkgName,
                v.InstalledVersion,
                v.FixedVersion || 'NO_FIX',
                v.VulnerabilityID
            ]);

            autoTable(doc, {
                startY: (doc as any).lastAutoTable.finalY + 20,
                head: [['Severity', 'Package', 'Installed', 'Fixed Version', 'CVE ID']],
                body: tableData,
                styles: {
                    fontSize: 9,
                    font: 'helvetica',
                ...
                didParseCell: (data) => {
                    if (data.section === 'body' && data.column.index === 0) {
                        ...
                        else if (sev === 'LOW') data.cell.styles.textColor = [37, 99, 235];
                    }
                }
            });
            ...
            doc.save(`${name}-${selectedTag}-security_audit.pdf`);

        } catch (e) {
            console.error("PDF Gen failed", e);
            setAlertMessage("PDF_GENERATION_FAILURE");
        }
    };
    ...
    useEffect(() => {
        if (scanStatusData?.data?.status === 'completed') {
            queryClient.invalidateQueries({ queryKey: ['manifest', name, selectedTag] });
            queryClient.invalidateQueries({ queryKey: ['scanHistory', name, selectedTag] });
        }
    }, [scanStatusData?.data?.status, name, selectedTag, queryClient]);
    ...
    // Derive Real Metrics
    const createdDate = details?.createdAt ? new Date(details.createdAt) : new Date();
    const ageInDays = Math.floor((new Date().getTime() - createdDate.getTime()) / (1000 * 3600 * 24));

    let pullFrequencyLabel = "LOW DENSITY";
    if (details?.pullCount && details.pullCount > 50) pullFrequencyLabel = "HIGH DENSITY";
    else if (details?.pullCount && details.pullCount > 10) pullFrequencyLabel = "MODERATE";
    ...
    const isScanningLocked = !allowedFeatures.includes('scanning');

    return (
        <div className="space-y-10 pb-20 relative">
            {alertMessage && (
                <div className="fixed bottom-10 right-10 bg-red-600 text-white px-6 py-4 rounded-xl shadow-2xl z-50 animate-in slide-in-from-right fade-in duration-300 font-bold uppercase tracking-widest flex items-center gap-4">
                    <ShieldAlert size={24} />
                    {alertMessage}
                    <button onClick={() => setAlertMessage(null)} className="ml-4 hover:text-red-200"><X size={16} /></button>
                </div>
            )}
            ...
            <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6">
                <div>
                    <div className="flex items-center gap-4 mb-2">
                        <Link to="/app/repositories" className="text-gray-500 hover:text-blue-400 transition-colors">
                            <ArrowLeft size={20} />
                        </Link>
                                        ...
                                        <div className="p-8 space-y-8">
                                            ...
                                            <div className="grid grid-cols-2 gap-4">
                                                <button
                                                    ...
                                                    className="col-span-2 flex items-center justify-center gap-3 px-6 py-4 bg-blue-600 text-white rounded-2xl font-black uppercase text-sm tracking-widest hover:bg-blue-500 transition-all shadow-[0_0_20px_rgba(37,99,235,0.3)]"
                                                >
                                                ...
                                                <button
                                                    ...
                                                    className="flex items-center justify-center gap-2 px-4 py-4 bg-white/5 text-gray-400 rounded-2xl font-black uppercase text-[10px] tracking-widest hover:bg-white/10 transition-all border border-white/5"
                                                >
                                            ...
                                            {scanStatus.status !== 'scanning' && (
                                                <button
                                                    onClick={isScanningLocked ? () => navigate('/app/settings') : handleTriggerScan}
                                                    className={clsx(
                                                        "w-full flex items-center justify-center gap-3 px-6 py-3 rounded-2xl font-black uppercase text-[9px] tracking-[0.3em] transition-all border",
                                                        isScanningLocked
                                                            ? "bg-white/5 text-gray-500 border-white/5 hover:bg-white/10 cursor-not-allowed"
                                                            : "bg-white/5 text-green-500 border-green-500/20 hover:bg-green-500/10 cursor-pointer"
                                                    )}
                                                >
                                                    {isScanningLocked ? <LockIcon size={14} /> : <RefreshCw size={14} />}
                                                    {isScanningLocked ? "Upgrade to Unlock Scanning" : "Re-Initialize Scan Protocol"}
                                                </button>
                                            )}
                                        </div>
                                    </div>
                                        ...
                                        {scanStatus.status === 'scanning' ? (
                                            <div className="flex flex-col items-center justify-center py-10 space-y-4 animate-in fade-in duration-500">
                                                <div className="relative">
                                                    ...
                                                    <RefreshCw size={48} className="text-blue-400 animate-spin relative z-10" />
                                                </div>
                                                <div className="space-y-1 text-center">
                                                    <div className="text-sm font-black text-white uppercase tracking-widest">Calibrating</div>
                                                    <div className="text-[10px] font-mono text-blue-400 uppercase tracking-widest animate-pulse">Analyzing Threat Vectors...</div>
                                                </div>
                                            </div>
                                        ) : details.healthScore && (
                                            <div className="space-y-6">
                                                ...
                                                <div className="pt-4 border-t border-white/5 grid grid-cols-2 gap-4">
                                                    <div className="space-y-1">
                                                        ...
                                                        <div className="text-xs text-white font-mono">{ageInDays} Cycles</div>
                                                    </div>
                                                    <div className="space-y-1">
                                                        <div className="text-[8px] text-gray-600 font-black uppercase">Pull Frequency</div>
                                                        <div className="text-xs text-white font-mono">{details.pullCount || 0} OPS ({pullFrequencyLabel})</div>
                                                    </div>
                                                </div>
                                            </div>
                                        )}
...
function VulnerabilityMatrix({ report }: { report: TrivyReport }) {
    // Deduplicate vulnerabilities based on ID + Package + Version
    const uniqueVulns = new Map<string, TrivyVulnerability>();
